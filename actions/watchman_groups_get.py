...
class WatchmanGroupsGet(BaseAction):
    def __init__(self, config):
        super(WatchmanGroupsGet, self).__init__(config)
    ...
    def run(
        ...
        url = f"https://{watchman_server}/v2.5/groups"
        params = {
        ...
        self.logger.info(
            f"Fetching groups from Watchman: {watchman_server}"
        )
        ...
        try:
            ...
            self.logger.info(
                f"Completed: {result['total_items']} groups "
                f"in {result['pages_posted']} requests"
            )

            return {
                'success': True,
                'group_count': result['total_items'],
            ...
            self.logger.error(
                f"Failed to process groups: {str(e)}"
            )
            try:
                error_payload = {
                    'success': False,
